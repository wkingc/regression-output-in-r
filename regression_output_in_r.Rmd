...
The regression starts with re-fitting the models with the problem points removed.  Finally, we use the <i>stargazer</i> function to print the results. To customzie the disply the results, we also need a bit of css which can be downloaded <a href="./regression_output_in_r.css" target="_blank">here</a>.  The <i>id</i> for the CSS is generated by adding <i>{#regression}</i> to the header of this section.  Check the <a href="./regression_output_in_r.Rmd" target="_blank">markdown</a> to see it in context.
...
Using copy and paste to summarize results leaves a ton of room to make errors.  To prevent this, we can summarize the results using inline R code.  Check out the <a href="./regression_output_in_r.Rmd" target="_blank">markdown</a> file for how I did it in this case.
